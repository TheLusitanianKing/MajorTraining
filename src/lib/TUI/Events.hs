...
selectEvent st =
  ...
  where
    focusedEquipment = F.focusGetCurrent $ st ^. apsFocusedEquipment
    stepIndex = F.focusGetCurrent $ st ^. apsFocusedStepIndex
    ...
    step' step eq
      | eq `Set.member` (step ^. stepEquipments) =
...
addStepEvent st
  | not . validNumberOfSteps $ length (st ^. apsCircuit . circuitSteps) + 1 =
    M.continue st
  | otherwise = M.continue $
    let
      circuit' = over circuitSteps (++ [Step Set.empty]) $ st ^. apsCircuit
...
removeStepEvent st
  | length (st ^. apsCircuit . circuitSteps) == 1 = M.continue st
  | otherwise = M.continue $
    let
      removeStepAt :: Int -> [a] -> [a]
      removeStepAt i xs = take i xs ++ drop (i+1) xs
      currentStep = fromMaybe 0 $ F.focusGetCurrent (st ^. apsFocusedStepIndex)
      circuit' = over circuitSteps (removeStepAt currentStep) $ st ^. apsCircuit
      nbSteps' = length $ circuit' ^. circuitSteps
    in
